...
$(document).ready(function(){
	$(".ch1").change(function(){// input class=ch1값을 가져옴
		if($(".ch1").is(":checked")){
				...
				//나오는순서
				if(arr.includes("service:"+input)){ //배열안에 해당하는 값이 있으면
				//	alert();
					wmsSource = ServicemakeSource("service:"+input,""); //서비스 구분목적값이 없으므로 ""값으로 보내줌
					crimerate_layers = makeLayer(wmsSource);
					crimerate_layers.setZIndex(1);
					map.addLayer(crimerate_layers);// 맵화면에 seoul레이어 추가
				}
				if(arr.includes("service:cctv")){//배열안에 해당하는 값이 있으면
					wmsSource = ServicemakeSource("service:cctv",cctv_ins); //cctv서비스 구분목적값이 cctv_ins값을 보내줌
					cctv_layers = makeLayer(wmsSource);
					cctv_layers.setZIndex(4);
					map.addLayer(cctv_layers);// 맵화면에 seoul레이어 추가
				}
				if(arr.includes("service:crmprvlmp")){//배열안에 해당하는 값이 있으면
					wmsSource = ServicemakeSource("service:crmprvlmp",""); //서비스 구분목적값이 없으므로 ""값으로 보내줌
					crmprvlmp_layers = makeLayer(wmsSource);
					crmprvlmp_layers.setZIndex(3);
					map.addLayer(crmprvlmp_layers);// 맵화면에 seoul레이어 추가
				}
				if(arr.includes("service:police")){//배열안에 해당하는 값이 있으면
					wmsSource = ServicemakeSource("service:police",police_ins);//cctv서비스 구분목적값이 police_ins값을 보내줌
					police_layers = makeLayer(wmsSource);
					police_layers.setZIndex(4);
					map.addLayer(police_layers);// 맵화면에 seoul레이어 추가
				}
		...
		map.removeLayer(crimerate_layers);// 맵화면에 레이어 제거
    		...
    		changeGuArea();
    			...
    			//나오는순서
				if(arr.includes("service:"+input)){
					address2="";
					wmsSource = ServicemakeSource("service:"+input,"");
					crimerate_layers = makeLayer(wmsSource);
					crimerate_layers.setZIndex(1);
					map.addLayer(crimerate_layers);// 맵화면에 seoul레이어 추가
				}
				if(arr.includes("service:cctv")){
					address2="";
					wmsSource = ServicemakeSource("service:cctv",cctv_ins);
					cctv_layers = makeLayer(wmsSource);
					cctv_layers.setZIndex(4);
					map.addLayer(cctv_layers);// 맵화면에 seoul레이어 추가
				}
				if(arr.includes("service:crmprvlmp")){
					address2="";
					wmsSource = ServicemakeSource("service:crmprvlmp","");
					crmprvlmp_layers = makeLayer(wmsSource);
					crmprvlmp_layers.setZIndex(3);
					map.addLayer(crmprvlmp_layers);// 맵화면에 seoul레이어 추가
				}
				if(arr.includes("service:police")){
					...
					wmsSource = ServicemakeSource("service:police",police_ins);
					police_layers = makeLayer(wmsSource);
					police_layers.setZIndex(4);
					map.addLayer(police_layers);// 맵화면에 seoul레이어 추가
